...
import { createUserReport, geocodeLocation, predictRisk } from './lib/risk';
import type { ReportIssueType } from './lib/risk';
...
const Navbar = ({
  ...
  <nav className="fixed top-0 left-0 right-0 z-50 flex h-16 items-center justify-between border-b border-tertiary bg-white/80 px-6 backdrop-blur-md">
    ...
    <div className="flex items-center gap-4">
      ...
      <button className="p-2 text-secondary/60 hover:text-primary" onClick={onOpenAlerts}>
        <Bell className="h-5 w-5" />
      </button>
      <button className="p-2 text-secondary/60 hover:text-primary" onClick={onOpenSettings}>
...
const Sidebar = ({
  ...
  <aside className="fixed left-0 top-16 flex h-[calc(100vh-4rem)] w-64 flex-col justify-between border-r border-tertiary bg-[#F8FAF9] p-6">
    <div>
      ...
      <button
        className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-3 text-sm font-bold text-white hover:bg-primary/90"
        onClick={onNewReport}
      >
    ...
    <button
      className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-secondary/60 hover:bg-tertiary/50 hover:text-secondary"
      onClick={onSupport}
    >
...
const RiskMapPage = ({
  ...
  onTemporaryAction: (message: string) => void;
}) => {
  ...
  return (
    <div className="flex h-screen pt-16">
      <Sidebar
        ...
        onNewReport={() => onTemporaryAction('Started a new temporary report workspace.')}
        onSupport={() => onTemporaryAction('Support workflow is temporary. We will wire it to tickets later.')}
      />
          ...
          <div className="absolute bottom-6 left-6 right-6 flex items-center gap-4">
            <div className="flex flex-1 items-center gap-6 rounded-xl border border-tertiary bg-white/90 p-4 backdrop-blur-md">
              ...
              <div className="flex items-center gap-2 border-l border-tertiary pl-6">
                <button
                  className="rounded-lg p-2 hover:bg-tertiary"
                  onClick={() => onTemporaryAction('Layer selector is temporary; map overlays coming soon.')}
                >
                  <Layers className="h-4 w-4 text-secondary/60" />
                </button>
                <button
                  className="rounded-lg p-2 hover:bg-tertiary"
                  onClick={() => onTemporaryAction('Map settings are temporary; advanced controls coming soon.')}
                >
                  <Settings className="h-4 w-4 text-secondary/60" />
                </button>
              </div>
            </div>
          </div>
...
const SafetyReportPage = ({
  ...
  const [issueType, setIssueType] = useState<ReportIssueType>('pothole');
    ...
    <div className="flex h-screen pt-16">
      ...
      <main className="ml-64 flex-1 overflow-y-auto bg-[#F8FAF9] p-8">
        <div className="mx-auto max-w-5xl">
          ...
          <h2 className="mb-2 text-4xl font-display font-bold text-secondary">{report ? report.query : 'Search for a route to generate a report'}</h2>
          {report ? (
            <p className="mb-4 text-xs font-medium uppercase tracking-widest text-secondary/40">
              You searched: {report.rawQuery}
            </p>
          ) : null}
          ...
          <div className="grid gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <div className="mb-6 flex items-center justify-between">
                ...
                <button
                  ...
                  onClick={() => onTemporaryAction('Archive export is temporary; downloadable CSV/PDF export is next.')}
                >
                  Export Full Archive
                </button>
              </div>
            ...
            <div className="glass-card bg-tertiary/30 p-6">
              ...
              <div className="mt-8 rounded-xl border border-tertiary bg-white p-4">
                <h4 className="mb-2 text-xs font-bold uppercase tracking-widest text-secondary/50">
                  Submit Issue With Report
                </h4>
                <p className="mb-3 text-xs text-secondary/60">
                  Save a user-reported issue to the backend with the currently loaded road + coordinates.
                </p>
                <form
                  className="space-y-3"
                  onSubmit={async (event) => {
                    event.preventDefault();
                    if (!report) {
                      setIssueError('Load a report first, then submit an issue.');
                      setIssueMessage(null);
                      return;
                    }
                    if (issueDescription.trim().length < 4) {
                      setIssueError('Please enter at least 4 characters in the issue description.');
                      setIssueMessage(null);
                      return;
                    }
                    ...
                    try {
                      const saved = await createUserReport({
                        road_name: report.roadName || report.query,
                        issue_type: issueType,
                        description: issueDescription.trim(),
                        latitude: report.latitude,
                        longitude: report.longitude,
                      });
                      setIssueMessage(`Saved issue #${saved.id} at ${new Date(saved.createdAt).toLocaleString()}.`);
                      setIssueDescription('');
                    } catch (submitError) {
                      setIssueError(
                        submitError instanceof Error
                          ? submitError.message
                          : 'Could not save the issue right now.',
                      );
                    } finally {
                ...
                >
                  <label className="block text-[11px] font-semibold text-secondary/60">
                    Issue Type
                    <select
                      value={issueType}
                      onChange={(event) => setIssueType(event.target.value as ReportIssueType)}
                      className="mt-1 w-full rounded-lg border border-tertiary bg-white px-2 py-2 text-xs"
                    >
                  ...
                  <label className="block text-[11px] font-semibold text-secondary/60">
                    ...
                    <textarea
                      ...
                      placeholder="Describe what you observed on this road segment..."
                      rows={3}
                      className="mt-1 w-full resize-y rounded-lg border border-tertiary bg-white px-2 py-2 text-xs"
                    />
                  </label>
                  {issueError ? <p className="text-xs text-red-500">{issueError}</p> : null}
                  {issueMessage ? <p className="text-xs text-primary">{issueMessage}</p> : null}
                  <button
                    type="submit"
                    disabled={isSubmittingIssue || !report}
                    className="flex w-full items-center justify-center gap-2 rounded-lg bg-primary py-2 text-xs font-bold text-white hover:bg-primary/90 disabled:opacity-60"
                  >
                    <AlertTriangle className="h-4 w-4" />
                    {isSubmittingIssue ? 'Saving Issue...' : 'Save Issue To Backend'}
                  </button>
                </form>
              </div>
              <button
                className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-tertiary bg-white py-3 text-xs font-bold text-secondary hover:bg-tertiary"
                onClick={() => onTemporaryAction('PDF export is temporary; printable report export is coming soon.')}
              >
...
const DocumentationPage = () => (
  <div className="mx-auto max-w-5xl px-6 pb-20 pt-24">
    <div className="mb-4 text-[10px] font-bold uppercase tracking-widest text-primary">
      Temporary Documentation Hub
    </div>
    <h2 className="mb-4 text-4xl font-display font-bold text-secondary">
      Project Links & Integration References
    </h2>
    <p className="mb-8 text-secondary/60">
      This page is a temporary target for navigation buttons until full docs are implemented.
    </p>
    <div className="grid gap-4 md:grid-cols-2">
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="rounded-xl border border-tertiary bg-white p-5 hover:bg-tertiary/40"
      >
        <div className="mb-1 text-sm font-bold text-secondary">Backend Swagger (Local)</div>
        <div className="text-xs text-secondary/60">Open local FastAPI docs for `/risk` and `/reports`.</div>
      </a>
      <a
        href="https://road-report-ai-backend.onrender.com/docs"
        target="_blank"
        rel="noreferrer"
        className="rounded-xl border border-tertiary bg-white p-5 hover:bg-tertiary/40"
      >
        <div className="mb-1 text-sm font-bold text-secondary">Backend Swagger (Deployed)</div>
        <div className="text-xs text-secondary/60">Open deployed FastAPI docs on Render.</div>
      </a>
      <a
        href="https://github.com/djkeithly/Road-Report-AI-Frontend"
        target="_blank"
        rel="noreferrer"
        className="rounded-xl border border-tertiary bg-white p-5 hover:bg-tertiary/40"
      >
        <div className="mb-1 text-sm font-bold text-secondary">Frontend Repository</div>
        <div className="text-xs text-secondary/60">Open active frontend source and branches.</div>
      </a>
      <a
        href="https://github.com/djkeithly/Road-Report-AI-Backend/tree/Model-prototype"
        target="_blank"
        rel="noreferrer"
        className="rounded-xl border border-tertiary bg-white p-5 hover:bg-tertiary/40"
      >
        <div className="mb-1 text-sm font-bold text-secondary">Backend Repository</div>
        <div className="text-xs text-secondary/60">Open backend model/API code.</div>
      </a>
    </div>
  </div>
);
...
export default function App() {
  ...
  const temporaryAction = (message: string): void => {
  ...
  const handleSearchSubmit = async () => {
    ...
    try {
      ...
      const prediction = await predictRisk({
        ...
        road_name: geocode.roadName,
        segment: geocode.segment,
      });
      const score = Math.round(prediction.risk_score * 100);
      const tier = getRiskTier(score);
      setReport({
        query: geocode.displayName,
        rawQuery: trimmed,
        ...
        score,
        tier,
        summary: buildSummary(tier, geocode.displayName, prediction.message),
  ...
  const footerCopy = useMemo(
    () =>
      report
        ? `Live report loaded for ${report.query} (${report.roadName}) - score ${report.score}/100`
        : 'Road safety frontend with live backend-connected reporting.',
    [report],
  );
  ...
  const renderPage = () => {
    switch (activePage) {
      ...
      case 'map':
        return <RiskMapPage report={report} onTemporaryAction={temporaryAction} />;
      case 'reports':
        return (
          <SafetyReportPage
            report={report}
            isLoading={isLoading}
            error={error}
            onTemporaryAction={temporaryAction}
          />
        );
      case 'methodology':
  ...
  return (
    <div className={`sovereign-app theme-${theme} min-h-screen bg-[#F8FAF9]`}>
      <Navbar
        ...
        isLoading={isLoading}
        onOpenAlerts={() =>
          temporaryAction('Alerts panel is temporary. It will show live incidents and user submissions.')
        }
        onOpenSettings={() =>
          temporaryAction('Settings panel is temporary. Account/profile controls are coming soon.')
        }
      />
      {actionMessage ? (
        <div className="fixed left-1/2 top-20 z-50 -translate-x-1/2 rounded-full border border-tertiary bg-white px-4 py-2 text-xs font-medium text-secondary shadow-md">
          {actionMessage}
        </div>
      ) : null}
        ...
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-8 md:flex-row">
          ...
          <div className="flex gap-8">
            {['Privacy Policy', 'Terms of Service', 'API Access', 'Contact Support'].map((link) => (
              <button
                key={link}
                className="text-[10px] font-bold uppercase tracking-widest text-secondary/40 hover:text-primary"
                onClick={() => {
                  if (link === 'API Access') {
                    setActivePage('documentation');
                    temporaryAction('Opened temporary API documentation page.');
                    return;
                  }
                  temporaryAction(`${link} page is temporary and will be added soon.`);
                }}
              >
                {link}
              </button>
